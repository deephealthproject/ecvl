...
public:
    ...
    Dataset() {}
    Dataset(std::filesystem::path& filename);

private:
    std::map<std::string, int> features_map_;
    void DecodeImages(const YAML::Node& node, std::filesystem::path& root_path);
    void LoadImage(Sample& sample, std::filesystem::path& root_path);
