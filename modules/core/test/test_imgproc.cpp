...
TEST(Imgproc, Flipping)
{
    ...
    *x.Ptr({ 0,0,0 }) = 0;
    ...
    // Test 3x3 Image
    x = Image({ 3, 3, 1 }, DataType::uint8, "xyc", ColorType::GRAY);
    *x.Ptr({ 0,0,0 }) = 1; *x.Ptr({ 1,0,0 }) = 1; *x.Ptr({ 2,0,0 }) = 1;
    *x.Ptr({ 0,1,0 }) = 2; *x.Ptr({ 1,1,0 }) = 2; *x.Ptr({ 2,1,0 }) = 2;
    *x.Ptr({ 0,2,0 }) = 3; *x.Ptr({ 1,2,0 }) = 3; *x.Ptr({ 2,2,0 }) = 3;
    Flip2D(x, y);
    EXPECT_EQ(*y.Ptr({ 0,0,0 }), 3); EXPECT_EQ(*y.Ptr({ 1,0,0 }), 3); EXPECT_EQ(*y.Ptr({ 2,0,0 }), 3);
    EXPECT_EQ(*y.Ptr({ 0,1,0 }), 2); EXPECT_EQ(*y.Ptr({ 1,1,0 }), 2); EXPECT_EQ(*y.Ptr({ 2,1,0 }), 2);
    EXPECT_EQ(*y.Ptr({ 0,2,0 }), 1); EXPECT_EQ(*y.Ptr({ 1,2,0 }), 1); EXPECT_EQ(*y.Ptr({ 2,2,0 }), 1);
}